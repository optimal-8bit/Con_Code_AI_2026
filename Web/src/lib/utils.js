...
export function deriveMedicationPlan({
  frequencyText = "",
  durationText = "",
  reminderTimes = [],
} = {}) {
  ...
  if (everyHoursMatch) {
    ...
    const explicitTimesMatch = frequency.match(
      /(\d+)\s*(x|times?)\s*(daily|per\s*day)/,
    );
  ...
  const normalizedReminderTimes =
    cleanReminderTimes.length > 0
      ? cleanReminderTimes.slice(
          0,
          Math.max(timesPerDay, cleanReminderTimes.length),
        )
      : defaultReminderTimes(timesPerDay);

  const durationDays = parseDurationDays(durationText);
  const adjustedTimesPerDay = Math.max(
    timesPerDay,
